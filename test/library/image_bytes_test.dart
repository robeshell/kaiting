...
void main() {
  ...
  test('accepts multi-MB PNGs whose IEND sits 8 bytes from EOF', () {
    ...
    final large = Uint8List(size)
      ..setAll(0, tiny.sublist(0, 8)); // PNG signature
  ...
  test('resolves and validates relocatable artwork cache keys', () async {
    final dir = await Directory.systemTemp.createTemp('art-relative-');
    addTearDown(() => dir.delete(recursive: true));
    await initArtworkUriResolver(cacheDirectory: dir.path);

    final good = File('${dir.path}/good.png');
    await good.writeAsBytes(base64DecodePng1x1());
    expect(resolveArtworkUri('good.png'), good.uri);
    expect(artworkFileLooksValid('good.png'), isTrue);
    expect(artworkFileLooksValid('missing.png'), isFalse);
  });
}
