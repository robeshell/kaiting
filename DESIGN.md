...
- 正在播放桌面双栏 1:1：栏距 48、页边距 44（折叠 <780 收 24/24），封面 classic ≤340 / 黑胶 ≤440（折叠 360），**封面/黑胶在播放列内水平居中**，列内其余元素 start 对齐；超宽/超高窗口的双栏内容舞台限 1600×1080 并在可用区域居中，氛围背景与页面 chrome 仍铺满视口。
...
- 快速定位：生成 token `lib/core/brand_tokens.g.dart`（禁止手改）；主题兼容层 `lib/core/sound_theme.dart`；组件 `lib/presentation/widgets/sound_components.dart`；正在播放 `screens/now_playing_screen.dart`、`widgets/vinyl_record_art.dart`、`mini_player.dart`。
